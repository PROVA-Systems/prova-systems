...
const CACHE_VERSION = 'prova-v3700-mega86-final-polish';   // MEGA⁸⁶ Sprint-Final 2026-05-17: Block A Pilot-Blocker (A.1 Cross-Domain-Login verifiziert + Diagnose-Logging in lib/prova-legacy-bridge.js, A.2 Index/App-Split Audit-Doku, A.3 lib/prova-diktat-mode-guard.js NEU mit 4-fach-Defense + Mode-Indicator-Badge + audit-log-v1-Integration eingebaut in app.html + ortstermin-modus.html) + Block B Audit-Edges Phase B (3 Caller migriert: freigabe-logic + editor-gate + audit-source-tracker auf audit-log-v1) + Block C Bibliothek-Drawer (lib/prova-bibliothek-drawer.js NEU mit Cmd+B Hotkey + postMessage-Bridge, eingebaut in 5 Pages, bibliothek.html embedded-Mode) + Block D Trial-Onboarding-Tour (Migration 60 + lib/prova-onboarding-tour.js NEU mit 5-Step Carousel + FOUNDING-99 Coupon-Display + Auto-Trigger nach Register) + Block E Support-Inbox-Quick-Reply (admin-kpis.html Section + supabase/functions/send-support-reply NEU mit send-email-Wrapper + Audit-Log) + Block F Mobile-Sidebar-Audit (Resize-Listener bereits durch O1-FIX implementiert + Touch-Targets-Doku) + Block G UX-Schliffe (Kontrast-Audit + Stepper-Verify + Empty-States bestätigt). Sprint-Historie: docs/SW-VERSION-HISTORY.md.
