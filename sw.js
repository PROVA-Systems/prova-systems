...
const CACHE_VERSION = 'prova-v3220-mega73-cleanup-marathon';   // MEGA73 AUTONOM-CLEANUP-MARATHON 2026-05-15 (~17 Sub-Commits seit MEGA72): Phase 1 Quarantäne-Sanity-Check (share.html + legal/datenschutz-intern.html zurück; anforderung-unterlagen-erweitert + ortstermin-arbeitsblatt verified 0-refs → bleibt in _archiv/ → 9 finale Quarantäne-Files), Phase 2a (3 broken-Save-Files Write-Path: termine/rechnungen/beratung Supabase upsert+update), Phase 2b (5 P2-Files Read+Write: wertgutachten/baubegleitung/erechnung/jahresbericht/statistiken), Phase 3 B-write (audit_trail-Insert bei Freigabe + users-Tabelle SV-Profile-Load + users.onboarding_completed_at-Sync — alle Schema-verifiziert via supabase-migrations/01_schema_foundation.sql), Phase 4 Doku-Refresh (CLAUDE.md Sprint-Stand + ARCHITEKTUR-MASTER Counts-Update 313 HTML/115 Netlify/143 Edge). Voraus: v3211 MEGA72-Marathon (Phase A + B-mini Track 0/1 partial + D-quarantine) + v3204 MEGA70-1.2.4 (Schema-Fix). 12 *-logic.js Files jetzt 0 Airtable-Reads. Branch feat/mega73-cleanup-marathon, ~17 Sub-Commits. Siehe docs/MEGA73-CLEANUP-COMPLETE.md für Marcel-5-Punkte-Smoke-Test + Branch-Topology.
