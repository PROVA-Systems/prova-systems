...
const CACHE_VERSION = 'prova-v3204-mega70-phase-1-2-4-auftrag-insert-fix';   // MEGA70-Phase-1.2.4 (2026-05-13 19:35 GMT+2): PRODUCTION-STOPPER fixed — Auftrag-Insert Schema-Drift. app-logic.js Submit-Payload an Supabase-Schema angepasst: Adresse → objekt jsonb (adresse/plz/ort statt flat adresse_strasse/plz/ort die nicht im Schema existieren), Auftraggeber-Typ UI-Label → enum (privat/gewerbe/gericht/versicherung/behoerde/andere via Mapping), Auftraggeber-Name + Ortstermin + Beweisfragen → details jsonb, Schadenart → schadensart_label (echte Column), Schadensdatum → schadensstichtag. Plus: onboarding-create-demo.js objekt-Keys an Schema-Konvention (adresse statt adresse_strasse, objektart statt typ), docs/SCHEMA-DRIFT-AUDIT.md für Phase-1.4 Followup (Read-Paths in akte/dashboard/freigabe-logic.js lesen noch via Airtable-Proxy mit CapitalCase-Feldnamen — out of scope 1.2.4).   // MEGA70-Phase-1.2.2 (2026-05-13 16:00 GMT+2): Naming-Cleanup (stellungnahme.html → fachurteil.html, gutachterliche-stellungnahme.html → kurzstellungnahme.html, ~50 ref-updates inkl. tests + netlify.toml + _redirects mit 301 legacy) + Phase-Tracking (4 Files mit Phase-Setter 2/3/4 + Fall-Wechsel-Bug-Fix in akte-logic.js ladeFallKontext) + Pricing-Sync (PROVA-REGELN + AGB + ANWALT + 7 live HTMLs + Email-Templates: 149/279€ → 179/379€). Voraus MEGA70-Phase-1.1 v3200 (Kontrast-Token-Fix WCAG AA für --text3 6.35:1) + MEGA71 Datei-Kartographie (separater Branch docs/mega71-kartographie, Commit 96d5b9a).
