...
const CACHE_VERSION = 'prova-v3905-mega88-c-totp-sync-fix';   // MEGA⁸⁸-C TOTP-Sync-Fix 2026-05-18: Block A Migration 62 mit AFTER-Trigger auf auth.mfa_factors → public.users.totp_enabled-Sync + Backfill bestehender User + Block B generate-mfa-recovery-codes + verify-mfa-recovery-code lesen jetzt BEIDE Quellen (users.totp_enabled ODER auth.mfa_factors verified) — Defense-in-Depth gegen Drift + Block C setup-2fa.html nach Verify expliziter UPDATE auf users.totp_enabled=true (Fallback wenn Trigger fehlt) + bei disable2FA auch totp_enabled=false + Codes leeren. Sprint-Historie: docs/SW-VERSION-HISTORY.md.
