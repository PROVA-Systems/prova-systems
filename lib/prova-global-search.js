...
(function (global) {
  ...
  const ProvaGlobalSearch = {
    async open() {
      ...
      // MEGA⁷⁸ C.2: search() ruft jetzt das RPC public.global_search auf
      // (siehe Migration 52_mega78_global_search_function.sql). Eine Round-
      // Trip, alle Quellen, RLS-scope'd.
      async function search(query) {
        ...
        try {
          ...
          const r = await sb.rpc('global_search', {
            q_text: query,
            q_limit: 5,
            q_source_filter: activeFilter
          });
          if (r.error) {
            list.innerHTML = `<div class="gs-empty gs-err">Fehler: ${_esc(r.error.message)}</div>`;
            return;
          }
