...
(function (global) {
  ...
  async function widgetFaelligeFristen() {
    const sb = await _getSb();
    const sieben = new Date(Date.now() + 7 * 86400000).toISOString();
    const { data } = await sb.from('fristen')
      .select('id, titel, faellig_am, status, auftrag_id')
      .in('status', ['offen', 'verlaengert'])
      .lte('faellig_am', sieben)
    ...
    const body = data.map(f => {
      const due = new Date(f.faellig_am).getTime();
      ...
      return `<a class="dw-row ${cls}" href="/fristen.html?id=${f.id}">
        <span class="dw-row-text">${_esc(f.titel || 'Frist')}</span>
  ...
  async function widgetMahnungen() {
    const sb = await _getSb();
    const { data } = await sb.from('rechnungen')
      .select('id, rechnungsnr, kontakt_id, betrag_brutto, mahnung_stufe, faellig_am, status')
      .in('status', ['versendet', 'mahnung_1', 'mahnung_2', 'mahnung_3'])
      .order('faellig_am', { ascending: true })
      .limit(5);
    if (!data || data.length === 0) {
      return _renderTile({
