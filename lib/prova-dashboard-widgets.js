...
(function (global) {
  ...
  // ─── Widget: Heute (Mission-Control) — MEGA⁸¹ B.4 ───────────────
  // Termine heute + Fristen heute + Anzahl ungelesene Notifications.
  // Quelle: direkter Supabase-Client (termine, fristen) + RPC notifications_unread_count
  async function widgetHeute() {
    const sb = await _getSb();
    const today = new Date().toISOString().slice(0, 10);
    const [tRes, fRes, nRes] = await Promise.all([
      sb.from('termine')
        .select('id, titel, typ, uhrzeit_von, ort_name, status')
        .eq('datum', today)
        .is('deleted_at', null)
      ...
      sb.from('fristen')
        ...
        .lte('datum_soll', today)
        .in('status', ['offen', 'verlaengert'])
        .is('deleted_at', null)
        .order('datum_soll', { ascending: true })
        .limit(6),
      sb.rpc('notifications_unread_count').then(r => r.data ?? 0, () => 0)
    ]);
    const termine = tRes.data || [];
    const fristen = fRes.data || [];
    const unread = Number(nRes) || 0;

    const parts = [];

    if (termine.length === 0 && fristen.length === 0 && unread === 0) {
      parts.push('<div class="dw-empty">Heute steht nichts an. Guten Tag!</div>');
    } else {
      if (unread > 0) {
        parts.push(`<a class="dw-row" href="#" onclick="window.ProvaNotifications?.open(); return false;">
          <span class="dw-row-text">🔔 ${unread} ungelesene Benachrichtigung${unread === 1 ? '' : 'en'}</span>
        </a>`);
      }
      if (termine.length) {
        parts.push(`<div class="dw-section-label">⏰ Termine heute (${termine.length})</div>`);
        termine.forEach(t => {
          const time = t.uhrzeit_von ? String(t.uhrzeit_von).slice(0, 5) : 'ganztags';
          const label = t.titel || t.typ || 'Termin';
          const ort = t.ort_name ? ' · ' + t.ort_name : '';
          parts.push(`<a class="dw-row" href="/termine.html?id=${t.id}">
            <span class="dw-row-az">${_esc(time)}</span>
            <span class="dw-row-text">${_esc(label + ort)}</span>
          </a>`);
        });
      }
      if (fristen.length) {
        parts.push(`<div class="dw-section-label">📅 Fristen heute oder überfällig (${fristen.length})</div>`);
        const todayDate = new Date(today + 'T00:00:00').getTime();
        fristen.forEach(f => {
          const due = new Date(f.datum_soll + 'T00:00:00').getTime();
          const days = Math.floor((due - todayDate) / 86400000);
          const lbl = f.notiz || f.frist_typ || 'Frist';
          const meta = days < 0 ? `vor ${Math.abs(days)}T` : 'heute';
          const cls = days < 0 ? 'is-overdue' : 'is-today';
          parts.push(`<a class="dw-row ${cls}" href="/fristen.html?id=${f.id}">
            <span class="dw-row-text">${_esc(lbl)}</span>
            <span class="dw-row-meta">${meta}</span>
          </a>`);
        });
      }
    }
    ...
    return _renderTile({
      title: 'Heute', icon: '🎯',
      cta: '/termine.html', ctaLabel: 'Kalender',
      body: parts.join('')
    });
  }
  ...
  const ProvaDashboardWidgets = {
    async render({ container, widgets }) {
      ...
      const list = widgets || ['heute', 'aktive', 'fristen', 'mahnwesen', 'ki_stats', 'aktivitaet'];
      const map = {
        heute: widgetHeute,
      ...
      el.dataset.provaWidgets = list.join(',');
    },
    async refresh(container) {
      const el = typeof container === 'string' ? document.querySelector(container) : container;
      if (!el) return;
      const list = (el.dataset.provaWidgets || 'heute,aktive,fristen,mahnwesen,ki_stats,aktivitaet').split(',');
      return this.render({ container: el, widgets: list });
    }
  };
