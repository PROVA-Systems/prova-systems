...
(function () {
  ...
  async function renderSkizzen(pane, fallId) {
    ...
    try {
      ...
      // M³⁹ P4: Canvas-Skizzen-Editor-Link (statt nur SVG-Page)
      const action = '<a href="/skizzen.html?az=' + encodeURIComponent(fallId) + '&nr=1" class="sft-action">+ Skizze erstellen (Canvas)</a>';
      if (!data.skizzen || !data.skizzen.length) {
        pane.innerHTML = action + '<div class="sft-empty">Noch keine Skizzen.</div>';
        return;
      }
      // M³⁹ P4: Render-Pfad unterscheidet SVG-Legacy vs Canvas-M³⁹P3
      const grid = data.skizzen.map(s => {
        const isCanvas = s.source === 'canvas';
        const linkHref = isCanvas
          ? `/skizzen.html?az=${encodeURIComponent(fallId)}&nr=${encodeURIComponent(s.skizze_nr || 1)}`
          : `/skizzen.html?id=${encodeURIComponent(s.id)}`;
        const markerBadge = isCanvas && s.marker_count > 0
          ? `<span style="background:#ef4444;color:#fff;border-radius:10px;padding:1px 6px;font-size:10px;font-weight:700;margin-left:4px;">📍 ${s.marker_count}</span>`
          : '';
        return `
          <a href="${linkHref}" style="text-decoration:none;color:inherit;">
            <div class="sft-skz-thumb" data-svg-id="${s.id}" data-canvas="${isCanvas ? '1' : '0'}" data-image-url="${isCanvas && s.image_url ? escHtml(s.image_url) : ''}"></div>
            <div style="font-size:11px;color:var(--text2);margin-top:4px;text-align:center;" class="sft-truncate">${escHtml(s.titel)}${markerBadge}</div>
          </a>`;
      }).join('');
      ...
      data.skizzen.forEach(s => {
        ...
        if (!wrap) return;
        if (s.source === 'canvas' && s.image_url) {
          // M³⁹ P3 Canvas-Skizze als <img>
          wrap.innerHTML = `<img src="${escHtml(s.image_url)}" alt="${escHtml(s.titel)}" style="width:100%;height:auto;border-radius:6px;">`;
        } else if (s.svg_content) {
          // Legacy SVG inline
          wrap.innerHTML = s.svg_content;
        } else {
          wrap.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text3);font-size:11px;">— kein Vorschau-Bild —</div>';
        }
      });
    } catch (e) {
      pane.innerHTML = '<div class="sft-empty" style="color:#f59e0b;">Fehler: ' + escHtml(e.message) + '</div>';
