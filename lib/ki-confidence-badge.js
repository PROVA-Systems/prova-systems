...
(function () {
  ...
  function compute(openaiResult, opts) {
    ...
    // 5. Model-Mapping (MEGA²⁸ W3-I0 — GPT-5.x + Claude 4.x)
    const model = (openaiResult.model || '').toLowerCase();
    const isFrontier = /gpt-5\.5|claude-opus-4-7|gpt-4o(?!-mini)|gpt-4(?!o-mini)/.test(model);
    const isLight = /gpt-5\.4-mini|gpt-5\.4-nano|gpt-4o-mini|haiku|gpt-3\.5/.test(model);
    if (isFrontier && !isLight) {
      if (reasons.length === 0) score = Math.min(100, score + 5);
    } else if (isLight) {
      if (opts.requireKonjunktivII) {
        score -= 20;
        reasons.push('Light-Modell (' + model + ') bei Konjunktiv II ungeeignet');
      }
    }
