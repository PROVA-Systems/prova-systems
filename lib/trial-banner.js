...
(function(){
  ...
  function _renderBanner(){
    ...
    // MEGA88-D Block B: Founding-Member-Variante (90d Trial + 99€ lifetime)
    var foundingStatus = localStorage.getItem('prova_founding_status_pending') || '';
    var isFounding = foundingStatus === 'founding_member' || foundingStatus === 'pilot_tester';
    ...
    var msg = days === 0 ? 'Trial endet HEUTE'
      ...
      : (isFounding ? 'Noch ' + days + ' Tage Founding-Trial' : 'Trial endet in ' + days + ' Tagen'));
    var icon = urgent ? '🚨' : (isFounding ? '🌟' : '⏳');
    var tagline = isFounding
      ? ' — danach 99 €/mo lifetime (Coupon auto)'
      : ' — sichere deinen PROVA-Zugang';
    var html = '<span>' + icon + ' <strong>' + msg + '</strong>' + tagline + '</span>'
