...
export const kontakte = {
    async list({ search = null, typ = null, limit = 100, offset = 0 } = {}) {
        ...
        // K-UI/X2: Select erweitert um Adress-Felder (DIN 5008) +
        // Conditional-Felder (kanzlei/versicherungs_nr/schaden_nr/behoerden_az)
        // + abteilung — fuer Liste-Anzeige + briefe-Empfaenger-Picker.
        let q = supabase
            .from('kontakte')
            .select(
                ...
                'email, telefon, mobil, ' +
                'kanzlei, versicherungs_nr, schaden_nr, behoerden_az, ' +
                'tags, created_at',
                { count: 'exact' }
            )
            .eq('workspace_id', wsId)
