...
// ═════════════════════════════════════════════════════════════
// BRIEFE — Korrespondenz-Generation (K-01..K-09 via brief-generate)
// ═════════════════════════════════════════════════════════════

export const briefe = {
    /**
     * Wrapper um supabase.functions.invoke('brief-generate').
     * Edge Function laedt users.letterhead_config + ruft PDFMonkey + speichert PDF
     * in Storage + dokumente-Tabelle. Frontend bekommt nur signed pdf_url zurueck.
     *
     * @param {Object} opts
     * @param {string} opts.template_key  — KORRESPONDENZ_TEMPLATES key (z.B. 'mahnung-1')
     * @param {Object} opts.variables     — PDFMonkey-Liquid-Variables
     * @param {string} [opts.contact_id]  — kontakt_id fuer dokumente-Insert
     * @param {string} [opts.auftrag_id]  — auftrag_id fuer dokumente-Insert
     * @param {string} [opts.az]          — Aktenzeichen fuer Storage-Path
     * @returns {Promise<{data, error}>}  — data: { dokument_id, pdf_url, file_path, bytes }
     */
    async generate(opts) {
        return await supabase.functions.invoke('brief-generate', { body: opts });
    }
};
