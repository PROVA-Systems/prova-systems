...
export const auftraege = {
    ...
    /**
     * Sprint 06b: Convenience-Wrapper fuer den Auftrags-Wizard.
     * Setzt explizit status='entwurf' (DB-ENUM auftrag_status default)
     * + phase_aktuell=1, damit Wizard-Drafts in einer separaten Liste
     * auffindbar sind (siehe listDrafts()).
     *
     * Nutzt unter der Haube create() — alle Validations + Trigger
     * (generate_az, search_vector) greifen weiter.
     *
     * @param {Object} data — Wizard-Payload (Stammdaten + Vorgangsdaten +
     *                       Objekt + Schadensart, abh. von typ).
     *                       Spalten die noch nicht in DB sind (siehe
     *                       SCHEMA-GAP-AUDIT.md, Sprint 06b PLANNED-
     *                       Migration) werden vom Caller weggefiltert.
     */
    ...
    /**
     * Sprint 06b: Liste der Entwurfs-Auftraege fuer Wizard-Recovery.
     * Ein Wizard-Draft kann nach Browser-Reload wieder geoffnet werden.
     */
...
export const kontakte = {
    async list({ search = null, typ = null, limit = 100, offset = 0 } = {}) {
        ...
        // K-UI/X2 + X2-Korr: Select enthaelt Stammdaten + Adress-Felder
        // (DIN 5008). Vorgangsdaten (versicherungs_nr/schaden_nr/
        // behoerden_az) gehoeren in den Auftrag — nicht selektiert.
        // kanzlei bleibt im Select fuer Legacy-Daten (Pre-Korrektur),
        // wird aber im UI nicht mehr angezeigt.
        let q = supabase
            .from('kontakte')
            .select(
                'id, typ, anrede, titel, vorname, nachname, firma, abteilung, name, ' +
                'adresse_strasse, adresse_nr, adresse_zusatz, plz, ort, land, ' +
                'email, telefon, mobil, kanzlei, ' +
                'tags, created_at',
                { count: 'exact' }
            )
            .eq('workspace_id', wsId)
...
export const cockpit = {
    async masterUebersicht() {
        // View ist Founder-global (1 Row, kein workspace_id-Filter noetig)
        return await supabase
