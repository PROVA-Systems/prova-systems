...
(function () {
  ...
  /**
   * MEGA¹⁷-PERFECTION W58: Effective-Mode mit Mobile-Fallback.
   *
   * Mode C ist auf Mobile nicht voll nutzbar (.docx-Upload erschwert,
   * Variable-Mapping-UI braucht Breite). Bei mobile + Mode-C-Default →
   * fallback auf 'A', wobei Source 'mobile-fallback' kennzeichnet.
   * Mode B bleibt mobile aktiv (TipTap funktioniert auf Touch).
   *
   * @param {object} params { auftragOverride, userDefault, isMobile?:bool }
   * @returns {object} { mode, source: 'override'|'default'|'fallback'|'mobile-fallback' }
   */
  function effectiveMode(params) {
    params = params || {};
    const baseResult = resolve(params);

    // Mobile-Detection: explizit oder via window.innerWidth
    const isMobile = (typeof params.isMobile === 'boolean')
      ? params.isMobile
      : (typeof window !== 'undefined' && window.innerWidth && window.innerWidth < 768);

    if (isMobile && baseResult.mode === 'C') {
   ...
   * Backend-Endpoint: /netlify/functions/workflow-settings (MEGA¹⁵)
   *
  ...
  async function fetchSettings() {
    ...
    try {
      const res = await fetcher('/.netlify/functions/workflow-settings');
      if (!res.ok) {
   ...
   * Backend-Endpoint: PATCH /netlify/functions/workflow-settings (MEGA¹⁵)
   *
