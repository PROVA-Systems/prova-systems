...
(function(){
  ...
  var STEPS = [
    ...
    // MEGA88-D Block A: Step 5 wird dynamisch befüllt — siehe _resolveCouponStep().
    // Default-Inhalt ist für Standard-User (kein Coupon-Code visible).
    {
      title: 'Bereit für deinen ersten Fall',
      icon: '🚀',
      body: 'Du bist startklar! Lege deinen ersten Auftrag an, teste das KI-Diktat und erkunde die Funktionen. Bei Fragen: <a href="/hilfe.html" style="color:#4f8ef7;">Hilfe-Bereich</a> oder Support.',
      cta: 'Zur App',
      ctaTarget: '/dashboard.html'
      // KEIN coupon-Feld — wird via _resolveCouponStep() per workspace.founding_status nachgereicht
    }
  ];

  // MEGA88-D Block A: User-spezifischer Coupon NUR wenn Founding-Status verifiziert
  // Verhindert dass jeder Trial-User FOUNDING-99 sieht und einlöst.
  async function _resolveCouponStep() {
    try {
      if (!_supabase) {
      ...
      if (!uid) return;
      var wsMod = await import('/lib/supabase-client.js');
      var wsId = wsMod.getActiveWorkspaceId ? await wsMod.getActiveWorkspaceId() : null;
      if (!wsId) return;
      var { data: ws } = await _supabase.from('workspaces')
        .select('id, name, founding_status, stripe_coupon_assigned')
        .eq('id', wsId).maybeSingle();
      if (!ws) return;
      var isFounding = ws.founding_status === 'founding_member' || ws.founding_status === 'pilot_tester';
      if (isFounding && ws.stripe_coupon_assigned) {
        // Persönlicher Founding-Step — KEIN Code-Anzeige, nur Hinweis dass auto-applied
        STEPS[4] = {
          title: ws.founding_status === 'pilot_tester' ? 'Pilot-Tester · 90 Tage gratis' : 'Founding-Member · 90 Tage gratis',
          icon: '🌟',
          body: 'Dein Konto ist als <strong>' + (ws.founding_status === 'pilot_tester' ? 'Pilot-Tester' : 'Founding-Member') +
                '</strong> markiert. Du hast 90 Tage komplett kostenlos, danach gilt dein persönlicher Coupon ' +
                'der beim Checkout automatisch angewendet wird. Kein Code-Eingabe nötig.',
          cta: 'Zur App',
          ctaTarget: '/dashboard.html'
        };
      }
      // Standard-User: Step 5 bleibt der generische „Bereit für deinen ersten Fall"-Step
    } catch (_) { /* best-effort, Default-Step bleibt */ }
  }
  ...
  function start(){
    ...
    _currentStep = 0;
    _render();
    // MEGA88-D Block A: Step 5 nachgereicht — Founding-Member sehen personalisierten Step,
    // Standard-User behalten den generischen „Bereit für deinen ersten Fall"-Step.
    _resolveCouponStep().then(function(){
      if (_currentStep === 4) _render();
    }).catch(function(){});
  }
