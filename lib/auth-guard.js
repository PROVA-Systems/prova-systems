...
export function writeLegacyBridge(user, session) {
    ...
    try {
        ...
        // KEIN Bridge-Hack mehr. Wir schreiben den ECHTEN Supabase access_token
        // (asymmetric ES256 JWT) in prova_auth_token. Server-Side verifiziert
        // ihn via JWKS in netlify/functions/lib/supabase-jwt.js — Industry-Standard.
        // Token-Refresh passiert automatisch durch supabase-js
        // (autoRefreshToken:true in supabase-client.js); watchAuthState fängt
        // TOKEN_REFRESHED ab und schreibt den neuen access_token rein.
        const accessToken = session && session.access_token;
        if (accessToken) {
            localStorage.setItem('prova_auth_token', accessToken);
            console.info('[auth-guard] Bridge: real Supabase access_token written');
        } else {
            // Fallback: format-valider Token (sollte selten passieren —
            // supabase-js liefert session.access_token zuverlässig). Server
            // wird diesen Token NICHT akzeptieren (Verify failed) — nur
            // synchrone Inline-IIFE-Guards auf Hybrid-Pages werden zufrieden.
            ...
            const payload = {
                ...
                exp: nowSec + 24 * 3600,
                uid: user.id,
                source: 'supabase-bridge-fallback'
            };
            const fallbackToken = _b64urlEncode(payload) + '.fallback-no-session-' + (user.id || 'unknown');
            localStorage.setItem('prova_auth_token', fallbackToken);
            console.warn('[auth-guard] Bridge: kein session.access_token — Fallback geschrieben');
        }
...
export async function runAuthGuard(options = {}) {
    ...
    // Phase 2 Option C: session weiterreichen damit echter access_token
    // statt Fake-Bridge-Token in prova_auth_token landet.
    writeLegacyBridge(session.user, session);
    _resetLoopCounter();
...
export function watchAuthState(options = {}) {
    return supabase.auth.onAuthStateChange((event, session) => {
        if (event === 'SIGNED_OUT') {
            ...
            // Token wurde refreshed — neuer access_token muss in prova_auth_token
            // damit prova-fetch-auth.js bei naechstem Function-Call den frischen
            // Token sendet.
            writeLegacyBridge(session.user, session);
