...
(function () {
  ...
  async function _loadAndRender() {
    ...
    try {
      ...
      let html = _renderRows(_allRecords, _currentFunktion);
      // MEGA¹⁴ W25 Bug-Fix: Pagination-Hint wenn 200-Limit erreicht
      if (_allRecords.length >= 200) {
        html = '<div role="note" style="margin-bottom:12px;padding:8px 12px;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.3);border-radius:6px;font-size:11px;color:#f59e0b;">' +
          '⚠️ Zeige 200 neueste Aufrufe. Aelteren Aufrufe nicht im aktuellen Zeitraum sichtbar.' +
        '</div>' + html;
      }
  ...
  function _buildModal(isAuftragSpecific) {
    ...
    modal.innerHTML =
      '<div class="prova-kih-modal" role="document">' +
        ...
        '<div class="prova-kih-toolbar">' +
          ...
          '</select>' +
          // MEGA¹⁴ W25 Bug-Fix: Refresh-Button hinzugefuegt
          '<button id="prova-kih-refresh" class="prova-kih-filter" aria-label="Aktualisieren" style="cursor:pointer;margin-left:auto;">🔄 Aktualisieren</button>' +
        '</div>' +
    ...
    // MEGA¹⁴ W25 Bug-Fix: Refresh-Button Event-Wiring
    const refreshBtn = modal.querySelector('#prova-kih-refresh');
    if (refreshBtn) refreshBtn.addEventListener('click', function () { _loadAndRender(); });
