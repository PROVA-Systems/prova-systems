 ...
 *   - PATCH /netlify/functions/workflow-settings { default_mode }
...
(function () {
  ...
  async function maybeShow() {
    ...
    if (!force) {
      // MEGA¹⁹ W78: DB-based Trigger — checkt user_workflow_settings via API.
      // Authoritative-Source ist die DB, nicht localStorage (das hilft nur als
      // Browser-Cache fuer Sessions ohne erfolgreiche API-Call).
      try {
        if (window.ProvaWorkflowMode && window.ProvaWorkflowMode.fetchSettings) {
          ...
          // settings._fallback === true bedeutet API hat 404/500 geliefert →
          // wir wissen nicht ob User onboarded ist → kein Modal (sonst wuerde
          // jeder API-Outage zum Modal-Spam fuehren). Skip ist defensive.
          if (settings && settings._fallback) {
            return;
          }
          // settings.default_mode gesetzt → User hat schon gewaehlt → done markieren
          if (settings && settings.default_mode) {
            _markDone();
            return;
          }
          // settings.default_mode null/undefined + kein _fallback → Modal zeigen
        }
      } catch (_) {
        // Network/Timeout: defensive skip statt Modal-Spam
        return;
      }
    }
  ...
  function _renderModal() {
    ...
    // MEGA¹⁹ W78: Backdrop-Blur + Mobile-responsive padding
    overlay.style.cssText = [
      ...
      'animation:provaOnbFadeIn 0.25s ease-out'
    ].join(';');

    // Inject Animation-Keyframes (1x)
    if (!document.getElementById('prova-onb-styles')) {
      const styleEl = document.createElement('style');
      styleEl.id = 'prova-onb-styles';
      styleEl.textContent =
        '@keyframes provaOnbFadeIn{from{opacity:0}to{opacity:1}}' +
        '@keyframes provaOnbScaleIn{from{transform:scale(0.95);opacity:0}to{transform:scale(1);opacity:1}}' +
        '#prova-onboarding-overlay > div{animation:provaOnbScaleIn 0.3s ease-out}' +
        '@media (max-width:540px){' +
          '#prova-onboarding-overlay > div{padding:20px 18px !important}' +
          '#prova-onboarding-overlay h2{font-size:19px !important}' +
          '#prova-onboarding-overlay [data-mode]{padding:14px !important}' +
        '}';
    ...
    // MEGA¹⁹ W78: A11y — Esc-Key zum Schliessen
    ...
    // MEGA¹⁹ W78: A11y — Click-outside (auf Backdrop, nicht auf Card)
    ...
    // MEGA¹⁹ W78: A11y — Focus-Trap (Tab cycelt nur in Buttons, kein Escape ins Background-DOM)
    const focusables = overlay.querySelectorAll('button:not([disabled])');
    if (focusables.length > 0) {
      // Initial-Fokus auf erste Mode-Card (nicht skip)
      const firstCard = overlay.querySelector('.prova-onb-card');
      if (firstCard) {
        setTimeout(() => firstCard.focus(), 50);
      }
  ...
  /**
   * Schliesst Overlay mit Cleanup (Esc-Handler + Animation).
   */
