...
(function (root, factory) {
  ...
  function _getConsentId() {
    // Persistent random UUID pro Browser für Audit-Trail-Korrelation
    var KEY = 'prova_cookie_consent_id';
    try {
      ...
      if (typeof crypto !== 'undefined' && crypto.randomUUID) {
        ...
        uuid = 'c-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 10);
      }
  ...
  function writeConsent(consent) {
    ...
    // MEGA⁴⁹: Bei analytics-Consent → Plausible nachträglich initialisieren
    // (storage-Event triggert nicht in same-tab, daher explicit)
    try {
      if (payload.analytics === true && typeof window !== 'undefined' && window.Plausible
          && typeof window.Plausible.init === 'function' && !window.Plausible.isEnabled()) {
        window.Plausible.init('prova-systems.de');
      }
    } catch (_) {}
    // MEGA⁴⁵-Fix: Body-Format an Edge-Function angepasst.
    // Edge erwartet { categories, consent_id, version }. Frühere Frontend-Form
    // { consent: payload } führte zu 400.
    try {
