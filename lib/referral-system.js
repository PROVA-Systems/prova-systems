...
(function (root, factory) {
  ...
  function getStats() {
    if (typeof fetch !== 'function') return Promise.reject(new Error('fetch not available'));
    const headers = (typeof window !== 'undefined' && window.provaAuthHeaders) ? window.provaAuthHeaders() : {};
    return fetch('/.netlify/functions/get-referral-stats', { headers }).then(r => r.json());
  }

  function getHistory() {
    if (typeof fetch !== 'function') return Promise.reject(new Error('fetch not available'));
    const headers = (typeof window !== 'undefined' && window.provaAuthHeaders) ? window.provaAuthHeaders() : {};
    return fetch('/.netlify/functions/get-referral-history', { headers }).then(r => r.json());
  }
