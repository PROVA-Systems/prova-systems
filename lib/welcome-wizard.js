...
(function () {
  ...
  async function maybeShow() {
    ...
    if (!force) {
      // DB-driven Trigger via existing /netlify/functions/user-workflow-settings
  ...
  async function _onNext(overlay) {
    ...
    if (step === 'persona') {
      // Save Persona via patch /netlify/functions/user-workflow-settings
      // (oder neuer Endpoint update-persona — wir nutzen existing-Pattern via PATCH)
      try {
        const fetcher = window.provaFetch || window.fetch;
        await fetcher('/.netlify/functions/user-workflow-settings', {
          method: 'PATCH',
