...
async function supSend(){var b=document.getElementById('sup-betreff').value.trim(),n=document.getElementById('sup-msg').value.trim();if(!b||!n){document.getElementById('sup-err').style.display='block';return;}document.getElementById('sup-err').style.display='none';var btn=document.getElementById('sup-btn');btn.disabled=true;btn.textContent='⏳ Wird gesendet...';try{await fetch('/.netlify/functions/make-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:'support',payload:{betreff:b,nachricht:n,sv_email:localStorage.getItem('prova_sv_email')||'',paket:localStorage.getItem('prova_paket')||'Solo',seite:window.location.pathname,ts:new Date().toISOString()}})});}catch(e){}document.getElementById('sup-form').style.display='none';document.getElementById('sup-faq-box').style.display='none';document.getElementById('sup-ok').style.display='block';}
...
// ── Einfügen-Modal Suche ──
window.filterEinfuegenListe = function() {
  var q = (document.getElementById('ei-search') || {}).value || '';
  q = q.toLowerCase();
  var items = document.querySelectorAll('#ei-liste .ei-item');
...
/* ── Fehlende Editor-Funktionen ── */
window.oeffneNeuEditor = function() {
  // Neuen Textbaustein erstellen — öffnet dasselbe Modal wie 'bearbeiteBaustein'
  if (typeof bearbeiteBaustein === 'function') {
    bearbeiteBaustein(null);
  } else if (typeof oeffneEditor === 'function') {
    oeffneEditor(null);
  } else {
    var modal = document.getElementById('tb-editor-modal') || document.getElementById('baustein-modal');
    if (modal) { modal.style.display = 'flex'; modal.classList.add('open'); }
    else if(typeof showToast==='function') showToast('Editor-Funktion nicht verfügbar', 'warn');
  }
};

window.schliesseEditor = function() {
  var modal = document.getElementById('tb-editor-modal') || document.getElementById('baustein-modal');
  if (modal) { modal.style.display = 'none'; modal.classList.remove('open'); }
};

window.schliesseEinfuegen = function() {
  var modal = document.getElementById('tb-einfuegen-modal');
  if (modal) { modal.style.display = 'none'; modal.classList.remove('open'); }
};

window.oeffneSupport = function() {
  if (typeof openSupportModal === 'function') openSupportModal();
  else { var fab = document.getElementById('sup-fab'); if(fab) fab.click(); }
};

window.schliesseSupport = function() {
  var fab = document.getElementById('sup-fab');
  if (fab) fab.click();
  var panel = document.querySelector('.sup-panel');
  if (panel) panel.classList.remove('open');
};

window.sendeTicket = async function() {
  var betreff  = (document.getElementById('ticket-betreff')  || {}).value || '';
  var nachricht = (document.getElementById('ticket-nachricht') || {}).value || '';
  var svEmail   = localStorage.getItem('prova_sv_email') || '';
  
  if (!betreff || !nachricht) {
    if(typeof showToast==='function') showToast('Bitte Betreff und Nachricht ausfüllen', 'warn');
    return;
  }
