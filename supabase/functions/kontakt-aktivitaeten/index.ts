...
Deno.serve(async (req) => {
  ...
  const auftraege = (await sb.from('auftraege')
    .select('id, az, titel, created_at')
    .eq('auftraggeber_kontakt_id', kontaktId)
    .is('deleted_at', null)
    .order('created_at', { ascending: false }).limit(limit)).data || [];
  const audit = (await sb.from('audit_trail')
    .select('id, action, entity_typ, entity_id, payload, created_at')
    .eq('entity_id', kontaktId)
    .order('created_at', { ascending: false }).limit(limit)).data || [];

  return J({ kontakt_id: kontaktId, auftraege, audit, total: auftraege.length + audit.length });
});
