...
function stripJpegExif(buf: Uint8Array): Uint8Array {
  ...
  while (i < buf.length - 1) {
    ...
    if (marker === 0xD9) { chunks.push(new Uint8Array([0xFF, 0xD9])); break; }
    if ((marker >= 0xD0 && marker <= 0xD7) || marker === 0x01) { chunks.push(buf.slice(i, i + 2)); i += 2; continue; }
    if (i + 4 > buf.length) break;
    const segLen = view.getUint16(i + 2);
    const segEnd = i + 2 + segLen;
    if (marker === 0xE1) { i = segEnd; continue; }
    if (marker === 0xDA) { chunks.push(buf.slice(i, buf.length)); break; }
    chunks.push(buf.slice(i, segEnd));
    i = segEnd;
  }
  let total = 0;
...
Deno.serve(async (req) => {
  ...
  if (!body.image_base64 || typeof body.image_base64 !== 'string') return J({ error: 'image_base64 (string) pflicht' }, 400);
  ...
  const path = `editor-images/${ms.workspace_id}/${crypto.randomUUID()}.${ext}`;
  ...
  try {
    const { data: ins } = await sb.from('document_images').insert({
      workspace_id: ms.workspace_id, user_id: user.id, storage_path: path, public_url: url,
      mime_type: mime, byte_size: bytes.length, alt: body.alt ?? null, caption: body.caption ?? null,
      original_filename: body.filename ?? null, exif_stripped: mime.startsWith('image/jp')
    }).select('id').maybeSingle();
    imageId = ins?.id ?? null;
  } catch { /* graceful */ }

  return J({
    image_id: imageId, url, alt: body.alt ?? null, caption: body.caption ?? null,
