...
Deno.serve(async (req) => {
  ...
  if (req.method !== 'DELETE' && req.method !== 'POST') return J({ error: 'Method Not Allowed' }, 405);
  ...
  if (!id) {
    try { const body = await req.json(); id = body?.id; } catch { /* graceful */ }
  }
  if (!id) return J({ error: 'id pflicht' }, 400);

  const { error } = await sb.from('eintraege').update({ deleted_at: new Date().toISOString() }).eq('id', id).is('deleted_at', null);
