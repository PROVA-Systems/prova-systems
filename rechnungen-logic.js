...
(function () {
  ...
  async function runF1(daten) {
    ...
    if (pdf && window.provaFetchAirtable && payload.rechnungsnummer) {
      try {
        var path = '/v0/appJ7bLlAHZoxENWE/tblF6MS7uiFAJDjiT?filterByFormula=' + encodeURIComponent('{Rechnungsnummer}="' + payload.rechnungsnummer.replace(/"/g, '\\"') + '"') + '&maxRecords=1';
        var g = await provaFetchAirtable({ method: 'GET', path: path });
        var j = await g.json();
        var rid = j.records && j.records[0] && j.records[0].id;
        if (rid) await provaFetchAirtable({ method: 'PATCH', path: '/v0/appJ7bLlAHZoxENWE/tblF6MS7uiFAJDjiT/' + rid, body: { fields: { PDF_URL: pdf } } });
      } catch (e) {}
    }
    if (pdf) {
      toast('PDF bereit');
