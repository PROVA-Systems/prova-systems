...
exports.handler = async (event) => {
  ...
  try {
    ...
    const docId = (doc && doc.document && doc.document.id);
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if ((status && status.document && status.document.status === 'success')) {
        pdfUrl = status.document.download_url;
        break;
      }
      if ((status && status.document && status.document.status === 'failure')) {
        return {
          statusCode: 200, headers,
