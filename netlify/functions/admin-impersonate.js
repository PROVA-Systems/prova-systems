...
/**
 * MEGA²³ Block 11: Best-effort Email-Notification an User bei Admin-Login.
 * Nutzt nodemailer mit den Standard-SMTP-ENVs (SMTP_HOST/USER/PASS/PORT).
 * Fire-and-forget — Failure darf den Impersonation-Flow NIE blocken.
 *
 * @param {object} opts { to, adminEmail, workspaceName, reason, ttlMinutes, adminIp, userAgent }
 * @returns {Promise<{ok:boolean, skipped?:string}>}
 */
async function notifyImpersonation(opts) {
  if (!opts || !opts.to || typeof opts.to !== 'string' || !/^[^@]+@[^@]+\.[^@]+$/.test(opts.to)) {
  ...
  const subject = '[PROVA] Admin-Login auf Ihrem Workspace';
  const text = [
    'Hallo,',
    '',
    'ein PROVA-Administrator (' + (opts.adminEmail || 'unbekannt') + ') hat sich gerade in Ihrem Workspace',
    '"' + (opts.workspaceName || '') + '" angemeldet (Read-Only-Modus, ' + (opts.ttlMinutes || 30) + ' Min).',
    '',
    'Grund: ' + (opts.reason || '— nicht angegeben —'),
    'IP-Adresse: ' + (opts.adminIp || 'unknown'),
    'User-Agent: ' + (opts.userAgent || 'unknown'),
    'Zeitpunkt: ' + new Date().toISOString(),
    '',
    'Der Admin kann KEINE Daten aendern (Read-Only-Token). Jede Aktion ist im Audit-Trail',
    'dokumentiert. Bei Fragen: support@prova-systems.de.',
    '',
    'PROVA Systems — DSGVO-Transparenz-Hinweis',
    '§ 32 BDSG · Art. 5 DSGVO (Transparency)'
  ].join('\n');
  await transporter.sendMail({
    from: process.env.SMTP_FROM || 'PROVA Admin <no-reply@prova-systems.de>',
    to: opts.to,
    subject: subject,
    text: text
  });
...
exports.handler = withSentry(requireAdmin(async function (event, context) {
  ...
  // MEGA²³ Block 11: Audit-Trail mit IP + User-Agent + Email-Notify
  const adminIp = (event.headers['x-nf-client-connection-ip'] || event.headers['client-ip'] || event.headers['x-forwarded-for'] || 'unknown');
  const userAgent = String(event.headers['user-agent'] || event.headers['User-Agent'] || 'unknown').slice(0, 500);
  try {
  ...
  if (process.env.IMPERSONATION_NOTIFY === 'on' && ws.billing_email) {
    notifyImpersonation({
      ...
      adminIp: adminIp,
      userAgent: userAgent
    }).catch(e => console.warn('[admin-impersonate] notify failed:', e.message));
  }
