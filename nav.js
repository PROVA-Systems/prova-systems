  ...
  */
  ...
  if (!document.getElementById('prova-nav-css')) {
    ...
    style.textContent = ''
      ...
      /* MEGA88-A: Brand-Logo via img (Schild+Checkmark) statt P-Block */
      + '.sb-logo-img{height:32px;width:auto;display:block;flex-shrink:0;}'
      + '.sb-logo-img.full{display:block;}'
      + '.sb-logo-img.icon{display:none;}'
      + '.sidebar.collapsed .sb-logo-img.full{display:none;}'
      + '.sidebar.collapsed .sb-logo-img.icon{display:block;height:28px;width:28px;}'
      /* Legacy-Markup-Compat: alte .sb-logo-mark/text Selektoren still vorhanden falls Drittcode darauf zugreift */
      + '.sb-logo-mark,.sb-logo-text{display:none;}'
  ...
  var html = ''
    ...
    +     '<img class="sb-logo-img full" src="/img/logo-prova.svg" alt="PROVA" height="32" width="auto">'
    +     '<img class="sb-logo-img icon" src="/img/logo-icon-only.svg" alt="PROVA" height="28" width="28">'
    +   '</a>'
