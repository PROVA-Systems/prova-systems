...
exports.handler = async function (event, context) {
  ...
  if (isSvTablePath(path)) {
    if (!internalOk) {
      ...
      const accessResult = await hasProvaAccess(String(user.email).trim().toLowerCase(), pat);
      const accessOk = typeof accessResult === 'boolean' ? accessResult : (accessResult && accessResult.ok);
      if (!accessOk) {
        return {
          statusCode: 403,
          body: JSON.stringify({ error: 'Kein Zugriff — Testphase beendet oder kein aktives Abo' })
        };
      }
    }
  }
