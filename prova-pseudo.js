/* ═══════════════════════════════════════════════════════════════════
   PROVA PSEUDONYMISIERUNG — prova-pseudo.js (Session 7)
   ═══════════════════════════════════════════════════════════════════
   
   ZWECK
   Ersetzt personenbezogene Daten im Diktat-Text BEVOR er an OpenAI
   (USA, Drittland) gesendet wird. Erforderlich für DSGVO-Konformität
   nach Art. 32 (technische Maßnahmen) und Art. 49 (Drittland-Übermittlung).
   ...
   Vollständige Adressen (Straße + Nr + PLZ + Ort) → [ADRESSE]
   Personen-Namen (zwei aufeinanderfolgende groß-Worte) → [PERSON]
                    — konservativ: nur wenn Kontext-Indikator
                      wie "Herr", "Frau", "Auftraggeber", "wohnhaft" davor
   
   WAS NICHT ERSETZT WIRD (bewusst — für KI-Fachurteil nötig):
   ─────────────────────────────────────────────────────────────
   - Messwerte (Prozente, Temperaturen)
   - Bau-Normen (DIN, WTA, ...)
   - Gebäude-Daten (Baujahr, Gebäudetyp)
   - Schadensart-Fachwörter
   - Datumsangaben
   ...
   AUDIT
   ─────
   ProvaPseudo.lastReport enthält pro Durchlauf:
     { iban: 1, telefon: 2, email: 1, adresse: 1, person: 1, length_in: 1200, length_out: 1180 }
   Das lässt sich in AUDIT_TRAIL schreiben für Nachweisbarkeit.
   ═══════════════════════════════════════════════════════════════════ */
...
(function(global) {
  ...
  // Mindestens 6 Ziffern, kein reiner Jahrgang/PLZ
  ...
  // ─── Straße + Hausnummer ───
  // Muster: "Musterstraße 42", "Am Bahnhof 3", "Zitronenweg 17a"
  // Vorsicht: deutsche Straßennamen enden oft auf -straße, -weg, -platz, -allee etc.
  ...
  // NUR ersetzen wenn direkt davor: Herr/Frau/Dr./Prof./Auftraggeber/wohnhaft/Herrn
  // Grund: Zu aggressive Namen-Erkennung löscht Firmenname "Allianz" oder "Schimmel Müller GmbH"
  //
  // Namen können aus bis zu 5 Teilen bestehen, getrennt durch Bindestrich ODER Leerzeichen:
  //   "Müller"                        — 1 Teil
  //   "Klaus Müller"                  — 2 Teile (Leerzeichen)
  //   "Klaus-Dieter Hartmann"         — 3 Teile (Bindestrich + Leerzeichen)
  //   "Klaus-Dieter Hartmann-Schönwalde"  — 4 Teile
  ...
  /**
   * Hauptfunktion: ersetzt PII durch Platzhalter
   */
  function apply(text) {
    ...
    out = out.replace(IBAN_INT_RE, function(match) {
      // Nur wenn Format plausibel und nicht schon ersetzt
    ...
    out = out.replace(TELEFON_RE, function(match) {
      // Mindestens 7 Ziffern gesamt für echte Nummer
    ...
    out = out.replace(PERSON_PREFIX_RE, function(match, name, offset, full) {
      report.person++;
      // Anrede behalten, Name ersetzen: "Herr Müller" → "Herr [PERSON]"
      const anrede = match.substring(0, match.length - name.length).trim();
      return anrede + ' [PERSON]';
    });

    // Zusammenfassen: Adresse-Anzahl
  ...
  /**
   * Reverse-Check: Prüft ob Payload noch sensible Daten enthält.
   * Gibt Array der Funde zurück (leer = sauber).
   */
  function audit(text) {
    if (!text) return [];
    const findings = [];
    if (IBAN_DE_RE.test(text)) findings.push('IBAN');
    IBAN_DE_RE.lastIndex = 0;  // Reset für globale Regex
  ...
  /**
   * Formatiert Report menschenlesbar
   */
  ...
  global.ProvaPseudo = ProvaPseudo;
