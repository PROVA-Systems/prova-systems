...
- **RLS-Read-Only-Lock-Pattern** (MEGA⁸⁹ Block A): Bei subscription-basierten SaaS NIEMALS auf Frontend-Logic für Trial-/Pausiert-Locks verlassen — DevTools/curl umgeht das in 30s. Pattern: zentrale `workspace_is_writable(uuid)` STABLE SECURITY DEFINER Helper-Function, in JEDER INSERT/UPDATE/DELETE/ALL-Policy aufrufen via `AND public.workspace_is_writable(workspace_id)`. **Read-Policies bleiben unverändert** (DSGVO-konforme Datenauskunft Art. 15/20). pg_cron-Job daily 02:00 UTC für Auto-Trial-Expiry — nicht in App-Logic. Leon-Lottermoser-Incident 18.05.2026 als Trigger: Trial 16 Tage offen + Read-Only nicht enforced → Hard-Lock via `auth.users.banned_until` (via `auth.admin.updateUserById ban_duration='720h'`) bei Suspicious-Activity.
- **Cron-Function-Founder-Exclude-Pattern** (MEGA⁸⁹ Hotfix 19.05.): Automatische Lock-/Cleanup-Cron-Jobs MÜSSEN Super-Admin-Workspaces explizit ausnehmen (`AND id NOT IN (SELECT wm.workspace_id FROM workspace_memberships wm JOIN users u ON u.id=wm.user_id WHERE wm.rolle='owner' AND u.is_founder=true)`). Sonst lockt sich Marcel selbst aus. Marcel-Restore-Incident: erster `cron_lock_expired_trials()`-Run hat Marcel's 2 eigene Workspaces (56e8ea96 + 65b25a13) pausiert weil abo_trial_endet_am abgelaufen war. Manual-Restore + Function-Hotfix in Migration 72. Lesson: vor jedem Production-Cron-Aktivieren `SELECT count(*) FROM <table> WHERE <cron-WHERE-clause>` ausführen + Owner-Founder-Exclude prüfen.
- **Login-Tracking-Architektur** (MEGA⁸⁹ Block C): `record_user_login(ip, ua, device, country)` als DB-Function statt App-Logic — atomarer Insert in user_sessions + Update users.last_login_at + audit_trail-Eintrag in einer Transaction. Frontend-Hook nach `_completeLogin` (NICHT nur nach signInWithPassword) sonst aal2-User nicht getrackt. Idempotenz via sessionStorage-Token-Hash (Page-Reload nach Login erzeugt 0 weitere Sessions). Plus: `lib/prova-session-heartbeat.js` mit 5min-Tick auf `users.last_active_at` + neueste `user_sessions.last_activity_at` — skippt bei `document.visibilityState='hidden'`, Tab-Focus triggert sofort-Tick.
- **Suspicious-Activity-Detection als VIEW** (MEGA⁸⁹ Block E): Pattern für SaaS-Konkurrenz-Reconnaissance: VIEW `suspicious_activity_v1` mit LATERAL-Joins aggregiert pro Workspace-Owner — `3+ Logins + 0 Aufträge + 7d alt = high_suspicion`, `1+ Login + 0 Aufträge + 14d alt = medium_suspicion`. Live-Query statt Materialized View (1×/Tag Cockpit-Check, akzeptable Latency, kein REFRESH-Cron nötig). HAVING-Clause filtert direkt im View → Frontend-Code minimal. 1-Click-Sperren-Button im Cockpit + Edge `admin-suspend-workspace` (Marcel-only via Email-Allow-List + Reason min 10 chars).
...
- **PDFmonkey-Bulk-Patch-Pattern** (MEGA-Marathon Phase 2.5): Statt 16+ Templates manuell im PDFmonkey-Dashboard zu klicken → REST-API-Tool `tools/pdfmonkey-bulk-patch.js` (Node 18+ native fetch, kein npm-Build). Pattern: `--dry-run` als Default, `--execute` macht Backup in `tools/pdfmonkey-backups/<ISO-timestamp>/` mit `_index.json` PRE-Patch, `--rollback=<dir>` restored via PATCH. Marker-Approach: HTML-Comments `<!-- PROVA-LOGO-HEADER -->` / `<!-- EU-AI-ACT-DISCLOSURE -->` in Templates erlauben idempotente Patches. Patches-Array easy erweiterbar. `PDFMONKEY_API_KEY` als Env-Var (niemals committen). Filter `--only=F-09` für single-Template-Test. Lesson: für jede externe SaaS-Plattform mit REST-API lieber CLI-Tool als Dashboard-Click-Marathon.
- **Tool-Idempotenz-Pattern** (MEGA-Marathon Phase 2.5c, Marcel-Bug 19.05.): Replacement-HTML darf NIEMALS den Marker enthalten, der das Replacement triggert — sonst Multiplikation bei Re-Runs (Marcel hatte nach 3 `--execute` Runs 2-3 Logos pro Template). Pattern: (1) im `LOGO_HEADER_HTML` keinen `<!-- PROVA-LOGO-HEADER -->` mehr, stattdessen `data-prova-component="logo-header"` als idempotenter Self-Marker für Detection. (2) Detection-Reihenfolge: **„schon vorhanden"-Check ZUERST** (skip mit Reason), dann Marker-Check, dann Auto-Inject. (3) Cleanup-Flags wie `--dedupe-logos` / `--dedupe-eu` für bestehende Duplikate aus früheren Buggy-Runs. (4) Identifier-Detection via Token-IN-Name statt Prefix wenn echte Template-Namen Stilangaben enthalten ("PROVA – F-09 – KURZGUTACHTEN" hat F-09 in der Mitte, nicht am Anfang).
- **Button-Design-System-Patterns** (MEGA-Marathon Phase 3): Token-System via CSS Custom Properties (`--pbtn-*`) — Light + Dark-Mode-Switch automatisch über `body.theme-dark` / `[data-theme="dark"]` / `html.dark`. Multi-Layer-Shadows (Stripe/Linear/Vercel-Style) statt flat: `0 1px 2px rgba()` + `0 1px 1px rgba()` default, beim Hover Upgrade auf größere Shadow + `translateY(-1px)`. Active = `translateY(0)` + `inset shadow`. Cubic-Bezier-Transition 150ms. Focus-visible nur per Keyboard (kein blauer Ring bei Mouse-Click). Loading-State per CSS-Spinner ohne JS. Mobile-Touch-Target ≥ 44px via `@media (hover:none) and (pointer:coarse)`. **Legacy-Compat-Bridge:** `.btn-primary` / `.es-btn` / `.cp-impersonate-btn` werden auf Tokens gemappt → globaler Visual-Upgrade ohne 200+ HTML-Files anzufassen. Pattern in `lib/prova-button-tokens.css` referenzierbar. `prefers-reduced-motion: reduce` deaktiviert Animations.
